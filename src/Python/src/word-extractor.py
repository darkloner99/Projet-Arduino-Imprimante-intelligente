...
def getWritting(image):
    ...
    for i, ctr in enumerate(sorted_ctrs):
        ...
        for j,ctr_1 in enumerate(sorted_ctrs_1):
            ...
            size = len(roi_1)
            if size > 300:   # apply filter: 100 character, 300 words
                path = outpath + str(count) + ".jpg"
                print(path)
                cv2.imshow('test',roi_1)
                #time.sleep(0.5)
